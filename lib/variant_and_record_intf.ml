...
module%template [@modality p = portable] Types = struct
  module type Tag = sig
    type ('variant, 'args) create : value mod contended portable =
      | Args of ('args -> 'variant) @@ portable
      | Const of (unit -> 'variant) @@ portable
    [@@unsafe_allow_any_mode_crossing]

    type ('variant, 'args) t : value mod contended portable
  end
  ...
  module type Field = sig
    type ('record, 'field) t : value mod contended portable
  end
