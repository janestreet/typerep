...
external opaque : 'a. 'a -> 'a = "%opaque" [@@layout_poly]
external obj_magic : 'a 'b. 'a -> 'b = "%identity" [@@layout_poly]

let double_array_value typerep =
  (* returns [None] if values of the same kind will never be floats. *)
  let rec aux : type a. a Typerep.Kind.t -> (unit -> a) option = function
    ...
    | Isomorphism (type b) (k : b Typerep.Kind.t) ->
