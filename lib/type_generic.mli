...
(** A computation is the type of an operation that can be applied to various different
    ...
    (** main function : get the computation from the typerep *) val of_typerep : 'a
    Typerep.t -> [ `generic of 'a computation ]
...
module type Named = sig
  ...
  module Context : sig
    (** Mutable context used to memorize some info during the traversal of a typerep. A
        new context is created before starting to enter the toplevel of a typerep. Then it
        is passed to all [init] calls that happen during the traversal of it. The user of
        the generic functor is free to stuff there whatever context needs to be available
        while creating a new value of type ['a Named.t] *)
    type t
  ...
  (** It might be interesting to inline some computation for a few typerep if they appear
      ...
      between multiple occurences of the same typename. [share = true] means no inlining.

      Note that not sharing recursive types will lead the [of_typerep] function to loop
      forever. Be careful when setting this.

      An example where it is not suitable to share everything for example is typestruct.
      The typestruct of an int is a simple constructor called [Int], naming it once and
...
module Ident : sig
  (** Runtime identifier for a generic computation. This is essentially a string whose
      ...
      The field called [required] is needed in order to build a generic computation
      module. It is used to establish a set up that would explicitly list all the
      computation that are required by an other computation to work.
...
module type S = sig
  ...
  (** Extending an existing generic for a particular type name
      ...
      variable of kind * -> k val register1 : 'a 't Typerep.t -> ('a computation -> 'a 't
      computation) -> unit ... *)
  ...
  (** special less scary type when the type has no parameters. this is equivalent as using
      register0 *)
...
(** The [name] is used for debug information only in case of Broken_dependency. The
    [required] is to handle dependencies between generics at runtime. Example: if [X] is
