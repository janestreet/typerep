module Helper (A : Intf.S) (B : Intf.S) = struct
      ...
      | A.Variant.Value (atag, a) ->
        (fun (type args) (atag : (variant, args) A.Tag.t) (a : args) ->
          ...
          (fun (type ex) (btag : (variant, ex) B.Tag.t) ->
            let Type_equal.T =
              Type_name.same_witness_exn (A.Tag.tyid atag) (B.Tag.tyid btag)
            in
      ...
      let get (type a) (afield : (_, a) A.Field.t) =
        ...
        | B.Record_internal.Field bfield ->
          (fun (type ex) (bfield : (record, ex) B.Field.t) ->
            let Type_equal.T =
              Type_name.same_witness_exn (A.Field.tyid afield) (B.Field.tyid bfield)
            in
...
module Make_named_for_closure (X : sig
  ...
  let init () name =
    let path = Type_name.Uid.name (Type_name.uid name) in
...
module Ident = struct
  type t = {
    name : string;
    implements : Type_name.Uid.t -> bool;
  }
          ...
          if not (implements uid) then begin
            ...
            let message =
              Printf.sprintf "Type_generic %S requires %S for uid %S\n"
                name name' (Type_name.Uid.name uid)
            in
...
module type S_implementation = sig
  ...
  type implementation = {
    generic : 'a. 'a Type.Rep.t -> 'a computation;
  }
  ...
  *)
  val find_extended_implementation :
    implementation -> 'a Type.Rep.Named.t -> 'a computation option
end
...
module Make_S_implementation(X : sig
  ...
  module Uid_table = struct
    include Hashtbl.Make(Type_name.Uid)
    let find table key =
  ...
  module Find0(T : Type.Rep.Named.T0) : sig
    val compute : unit -> T.named computation option
  end = struct
    let compute () =
      match Uid_table.find table0 (Type_name.uid T.typename_of_t) with
      | None -> None
      | Some rep ->
        let module S0 = (val rep : S0) in
        let witness = Type_name.same_witness_exn S0.typename_of_t T.typename_of_named in
  ...
  module Find1(T : Type.Rep.Named.T1) : sig
    val compute : unit -> (T.a computation -> T.a T.named computation) option
  end = struct
    let compute () =
      match Uid_table.find table1 (Type_name.uid T.typename_of_t) with
      | None -> None
      | Some rep ->
        let module S1 = (val rep : S1) in
        let module Conv = Type_name.Same_witness_exn_1(S1)(struct
          type 'a t = 'a T.named
  ...
  module Find2(T : Type.Rep.Named.T2) : sig
    val compute : unit
      -> (T.a computation
          -> T.b computation
    ...
    let compute () =
      match Uid_table.find table2 (Type_name.uid T.typename_of_t) with
      | None -> None
      | Some rep ->
        let module S2 = (val rep : S2) in
        let module Conv = Type_name.Same_witness_exn_2(S2)(struct
  ...
  module Find3(T : Type.Rep.Named.T3) : sig
    val compute : unit
      -> (T.a computation
          -> T.b computation
          -> T.c computation
    ...
    let compute () =
      match Uid_table.find table3 (Type_name.uid T.typename_of_t) with
      | None -> None
      | Some rep ->
        let module S3 = (val rep : S3) in
        let module Conv = Type_name.Same_witness_exn_3(S3)(struct
  ...
  module Find4(T : Type.Rep.Named.T4) : sig
    val compute : unit
      -> (T.a computation
          -> T.b computation
          -> T.c computation
    ...
    let compute () =
      match Uid_table.find table4 (Type_name.uid T.typename_of_t) with
      | None -> None
      | Some rep ->
        let module S4 = (val rep : S4) in
        let module Conv = Type_name.Same_witness_exn_4(S4)(struct
  ...
  module Find5(T : Type.Rep.Named.T5) : sig
    val compute : unit
      -> (T.a computation
          -> T.b computation
          -> T.c computation
    ...
    let compute () =
      match Uid_table.find table5 (Type_name.uid T.typename_of_t) with
      | None -> None
      | Some rep ->
        let module S5 = (val rep : S5) in
        let module Conv = Type_name.Same_witness_exn_5(S5)(struct
  ...
  let find_extended_implementation (type a) aux = function
    | Type.Rep.Named.T0 rep -> begin
      let module T = (val rep : Type.Rep.Named.T0 with type t = a) in
    ...
    | Type.Rep.Named.T1 rep -> begin
      let module T = (val rep : Type.Rep.Named.T1 with type t = a) in
    ...
    | Type.Rep.Named.T2 rep -> begin
      let module T = (val rep : Type.Rep.Named.T2 with type t = a) in
    ...
    | Type.Rep.Named.T3 rep -> begin
      let module T = (val rep : Type.Rep.Named.T3 with type t = a) in
    ...
    | Type.Rep.Named.T4 rep -> begin
      let module T = (val rep : Type.Rep.Named.T4 with type t = a) in
    ...
    | Type.Rep.Named.T5 rep -> begin
      let module T = (val rep : Type.Rep.Named.T5 with type t = a) in
  ...
  let _using_extended_implementation aux rep content =
    ...
    | None -> begin
      ...
      | None ->
        let typename = Type.Rep.Named.typename_of_t rep in
        let name = Type_name.Uid.name (Type_name.uid typename) in
        raise_not_implemented name
    end
end

module Key_table = Hashtbl.Make(Type_name.Key)
...
module Make(X : sig
  ...
  module Memo = Type_name.Table(struct type 'a t = 'a X.Named.t end)

  module Helper = Helper(Rep)(Computation)

  let of_typerep rep =
    let context = X.Named.Context.create () in
    let memo_table = Memo.create 32 in
    let rec of_typerep : type a. a Rep.t -> a t = function
      | Rep.Int -> X.int
      | Rep.Int32 -> X.int32
      | Rep.Int64 -> X.int64
      | Rep.Char -> X.char
      ...
      | Rep.Bool -> X.bool
      | Rep.Unit -> X.unit
      | Rep.Option rep -> X.option (of_typerep rep)
      | Rep.List rep -> X.list (of_typerep rep)
      | Rep.Array rep -> X.array (of_typerep rep)
      | Rep.Lazy rep -> X.lazy_t (of_typerep rep)
      | Rep.Ref rep -> X.ref_ (of_typerep rep)
      | Rep.Function (dom, rng) ->
        X.function_ (of_typerep dom) (of_typerep rng)
      | Rep.Tuple tuple -> begin
        ...
        | Rep.Tuple.T3 (a, b, c) ->
          ...
          X.tuple3 ra rb rc
        | Rep.Tuple.T4 (a, b, c, d) ->
          let ra = of_typerep a in
          let rb = of_typerep b in
          let rc = of_typerep c in
          let rd = of_typerep d in
          X.tuple4 ra rb rc rd
        | Rep.Tuple.T5 (a, b, c, d, e) ->
          let ra = of_typerep a in
          let rb = of_typerep b in
          let rc = of_typerep c in
          let rd = of_typerep d in
      ...
      | Rep.Named (named, content) -> begin
        let typename = Type.Rep.Named.typename_of_t named in
